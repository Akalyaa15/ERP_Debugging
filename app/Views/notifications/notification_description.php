...
if ($notification->dc_id && $notification->delivery_estimate_date) {
    $delivery_date = format_to_date($notification->delivery_estimate_date, FALSE);
   
    echo "<div>" . lang("date") . ": " . $delivery_date . "</div>";
}

if ($notification->voucher_id && $notification->voucher_expense_date) {
    $voucher_date = format_to_date($notification->voucher_expense_date, FALSE);
    echo "<div>" . lang("voucher_creater") . ": " . $notification->voucher_user_name . "</div>";  
    echo "<div>" . lang("voucher_no") . ": " . $notification->voucher_no . "</div>";  
    echo "<div>" . lang("date") . ": " . $voucher_date . "</div>";
} 


if ($notification->payslip_id && $notification->payslip_start_date) {
    $leave_date = format_to_date($notification->payslip_start_date, FALSE);
    /*if ($notification->payslip_start_date != $notification->leave_end_date) {
        $leave_date = sprintf(lang('start_date_to_end_date_format'), format_to_date($notification->payslip_start_date, FALSE), format_to_date($notification->payslip_end_date, FALSE));
    } */
    echo "<div>" . lang("date") . ": " . $leave_date . "</div>";
}
...
if ($notification->group_id && $notification->group_title) {
    echo "<div>" . lang("group") . ": " . $notification->group_title . "</div>";
}
if ($notification->group_comment_id && $notification->group_comment_title) {
    echo "<div>" . lang("comment") . ": " . convert_mentions($notification->group_comment_title, false) . "</div>";
}