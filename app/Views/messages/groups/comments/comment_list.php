                ...
                <?php if ($this->login_user->is_admin || $comment->created_by == $this->login_user->id) { ?>
                    <span class="pull-right dropdown comment-dropdown">
                        ...
                        <ul class="dropdown-menu" role="menu">
                            <li role="presentation"><?php echo ajax_anchor(get_uri("messages/group_delete_comment/$comment->id"), "<i class='fa fa-times'></i> " . lang('delete'), array("class" => "", "title" => lang('delete'), "data-fade-out-on-success" => "#prject-comment-container-$type-$comment->id")); ?> </li>
                        </ul>
                    </span>
                <?php } ?>
           ...
           <!--  <div class="comment-image-box clearfix"> -->
                ...
                <div class="mb15 clearfix">
                    ...
                    if ($type === "project" || $type === "customer_feedback") {
                        $can_reply = true;
                        echo ajax_anchor(get_uri("messages/group_comment_reply_form/" . $comment->id . "/" . $type . "/" . $type_id), "<i class='fa fa-reply font-11'></i> " . lang('reply'), array("data-real-target" => "#reply-form-container-" . $comment->id));
                    }
                    ?>
                    <?php
                    $reply_caption = "";
                    ...
                    if ($reply_caption) {
                        echo ajax_anchor(get_uri("messages/group_view_comment_replies/" . $comment->id), "<i class='fa fa-comment-o'></i> " . lang("view") . " " . $comment->total_replies . " " . $reply_caption, array("class" => "btn btn-default btn-xs view-replies", "id" => "show-replies-" . $comment->id, "data-remove-on-success" => "#show-replies-" . $comment->id, "data-real-target" => "#reply-list-" . $comment->id));
                    }

                    //create link for reply success. trigger this link after submit any reply
                    echo ajax_anchor(get_uri("messages/group_view_comment_replies/" . $comment->id), "", array("class" => "hide", "id" => "reload-reply-list-button-" . $comment->id, "data-real-target" => "#reply-list-" . $comment->id));
                    ...
                    if ($total_files) {
                        ...
                        echo anchor(get_uri("messages/group_download_comment_files/" . $comment->id), $download_caption, array("class" => "pull-right", "title" => $download_caption));
                    }
                    ?>
                </div>
            <!-- </div> -->
