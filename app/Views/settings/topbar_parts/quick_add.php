...
if ((($this->login_user->user_type == "staff"||$this->login_user->user_type == "resource") && ($this->login_user->is_admin || get_array_value($permissions, "can_manage_all_projects") == "1" || get_array_value($permissions, "can_create_tasks") == "1")) || ($this->login_user->user_type == "client" && get_setting("client_can_create_tasks"))) {
    //add tasks 
    ...
    //add multiple tasks
    $links .= modal_anchor(get_uri("projects/modal_form"), lang('add_project'), array("class" => "clearfix", "title" => lang('add_project'), "data-post-add_type" => "multiple", "id" => "js-quick-add-multiple-task"));
}
...
//add event
if (get_setting("module_event") == "1" && ((($this->login_user->user_type == "client"||$this->login_user->user_type == "vendor") && !in_array("events", $hidden_menu)) || $this->login_user->user_type == "staff"||$this->login_user->user_type == "resource")) {
...
//add note
if (get_setting("module_note") == "1" && ($this->login_user->user_type == "staff"||$this->login_user->user_type == "resource")) {
