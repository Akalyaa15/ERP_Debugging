...
<?php } ?>
<?php echo form_open(get_uri("income/save"), array("id" => "income-form", "class" => "general-form", "role" => "form")); ?>
<div id="income-dropzone" class="post-dropzone">
...
<div class="form-group" id="outsource" style="display: none">
       ...
       <?php
        ...
        <div class=" form-group">
            <label for="income_date" class=" col-md-3"><?php echo lang('income_date'); ?></label>
            <div class="col-md-9">
                <?php
                echo form_input(array(
                    "id" => "income_date",
                    "name" => "income_date",
                    "value" => $model_info->income_date? $model_info->income_date: get_my_local_time("Y-m-d"),
                    "class" => "form-control",
                    "data-rule-required" => true,
                    "data-msg-required" => lang("field_required"),
                ));
        ...
        <div class="form-group">
            <label for="income_project_id" class=" col-md-3"><?php echo lang('project'); ?></label>
            <div class=" col-md-9">
                <?php
                echo form_dropdown("income_project_id", $projects_dropdown, $model_info->project_id, "class='select2 validate-hidden' id='income_project_id'");
                ?>
            </div>
        </div>
        <div class="form-group">
 ...
 <div class="form-group"  id ="w">
        <label for="income_gst_number" class=" col-md-3"><?php echo lang('gst_number'); ?></label>
        <div class="col-md-9">
            <?php
            echo form_input(array(
                "id" => "income_gst_number",
                "name" => "income_gst_number",
    ...
    <div class="form-group">
        ...
        <div class="col-md-9">
            <?php
            echo form_input(array(
                "id" => "income_item_hsn_code",
                "name" => "income_item_hsn_code",
    ...
    <div class="form-group" id="z">
        <label for="income_item_hsn_description" class="col-md-3"><?php echo lang('hsn_description'); ?></label>
        <div class=" col-md-9">
            <?php
            echo form_textarea(array(
             "id" => "income_item_hsn_code_description",
            "name" => "income_item_hsn_code_description",
    ...
    <div class="form-group" id="a">
                <label for="income_recurring" class=" col-md-3"><?php echo lang('inclusive_tax'); ?>  <span class="help" data-toggle="tooltip" title="<?php echo lang('gst_applicable'); ?>"><i class="fa fa-question-circle"></i></span></label>
                <div class=" col-md-9">
                    <?php
                    echo form_radio(array(
...
$("#vendor_contact").select2({
     ...
     $("#client_contact,#vendor_contact").select2("readonly", true);
        ...
        var uploadUrl = "<?php echo get_uri("income/upload_file"); ?>";
        var validationUrl = "<?php echo get_uri("income/validate_income_file"); ?>";

        var dropzone = attachDropzoneWithForm("#income-dropzone", uploadUrl, validationUrl);

        $("#income-form").appForm({
            onSuccess: function (result) {
                if (typeof $INCOME_TABLE !== 'undefined') {
                    $INCOME_TABLE.appTable({newData: result.data, dataId: result.id});
                } else {
                    location.reload();
                }
            }
       ...
       $("#central_tax").click(function () {
        ...
        //setDatePicker("#income_date");

        $("#income-form .select2").select2();
...
$('#income_item_hsn_code').attr('readonly', true);
$("#payment_status").select2();
    });
function applySelect2OnHsnTitle() {
        $("#income_item_hsn_code").select2({
            showSearchBox: true,
            ajax: {
            ...
            if (e.val === "+") {
                ...
                $("#income_item_hsn_code").select2("destroy").val("").focus().attr('readonly', false);
                $("#income_item_gst").val("").attr('readonly', false);
                $("#income_item_hsn_code_description").val("").attr('readonly', false);
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (response && response.success) {
                            ...
                            if (!$("#income_item_gst").val()) {
                                $("#income_item_gst").val(response.item_info.gst);
                            }
                           if (!$("#income_item_hsn_code_description").val()) {
                                $("#income_item_hsn_code_description").val(response.item_info.hsn_description);
                            }
...
<script type="text/javascript">
    $("#income_item_hsn_code").on("change", function() {
   
        $("#income_item_gst").val("")
       
        $("#income_item_hsn_code_description").val("")
});
</script>
<script type="text/javascript">
    $("#close_hsn_code").on("click", function() {
   $("#income_item_hsn_code").val("").attr('readonly', false)
        $("#income_item_gst").val("")
...
<script type="text/javascript">
    ...
    $("#income_user_id").change(function () {
    $("#voucher_no").val("").attr('readonly', false)
                    var team_member =$("#income_user_id").val();

          $("#voucher_no").select2({
            showSearchBox: true,
            ajax: {
                url: "<?php echo get_uri("income/get_voucher_id"); ?>",
                dataType: 'json',
               data: function (term, page) {
                    return {
                        team_member: team_member // search term
        ...
        $("#income_user_ids").change(function () {
    $("#voucher_no").val("").attr('readonly', false)
                    var team_member =$("#income_user_ids").val();
        ...
        $("#income_user_idss").change(function () {
    $("#voucher_no").val("").attr('readonly', false)
                    var phone =$("#income_user_idss").val();
          $("#voucher_no").select2({
            showSearchBox: true,
            ajax: {
                url: "<?php echo get_uri("income/get_voucher_id_others"); ?>",
                dataType: 'json',
               data: function (term, page) {
                    return {
...
$("#client_contact").change(function () {
    $("#voucher_no").val("").attr('readonly', false)
          ...
          $("#voucher_no").select2({
            showSearchBox: true,
            ajax: {
                url: "<?php echo get_uri("income/get_client_voucher_id"); ?>",
                dataType: 'json',
               data: function (term, page) {
                    return {
                        team_member: team_member // search term
...
<script type="text/javascript">
     $("#voucher_no").change(function () {
        ...
        $.ajax({
                    url: "<?php echo get_uri("income/get_voucher_details"); ?>",
                    ...
                    success: function (response) {
                        ...
                        if (response && response.success) {
                           ...
                           /*$("#description").val(response.item_info.description);
                            $("#income_date").val(response.item_info.expense_date);
                            $("#category_id").select2("val", response.item_info.category_id);
                            $("#amount").val(response.item_info.amount);
                               $("#income_project_id").select2("val",response.item_info.project_id);
                               $("#income_user_id").select2("val",response.item_info.user_id);*/
$("#v_filess,#v_status").show()   
   $("#filess").html(response.item_files);
     $("#voucher_status").html(response.item_status);
                            ...
                            $("#income_date").val(response.item_info.expense_date).attr('readonly', true);
                            $("#category_id").select2("val", response.item_info.category_id);
...
<script type="text/javascript">
    $("#without_gst").on("click", function() {
        ...
        //$("#income_item_hsn_code").hide()
        $("#s").hide()
        $("#y").hide()
        $("#z").hide()
        $("#a").hide()
       $("#without_inclusive_tax").click()
        $("#income_item_hsn_code_description").hide()
        $("#income_item_gst").hide()
});
</script>
<script type="text/javascript">
    $("#with_gst").on("click", function() {
   
        //$("#income_item_hsn_code").show()
        $("#s").show()
        $("#y").show()
        $("#z").show()
       $("#a").show()
        $("#income_item_hsn_code_description").show()
        $("#income_item_gst").show()
});
</script>
<?php 
...
$("#client_member_contact").hide()
 ...
 <script >
    $("#income_user_id,#vendor_contact,#client_contact,#income_user_ids,#income_user_idss").change(function () {
$("#v_filess,#v_status").hide()   
         $("#description").val("").attr('readonly', false);
                            $("#income_date").val("").attr('readonly', false);
                            $("#category_id").select2("val", " ");
                            $("#amount").val("").attr('readonly', false);
                            $("#income_project_id").select2("val"," ");
...
$("#description").attr('readonly', true);
                            $("#income_date").attr('readonly', true);
                            $("#category_id").attr('readonly', true);
                            $("#amount").attr('readonly', true);
                               $("#income_project_id").attr('readonly', true);
