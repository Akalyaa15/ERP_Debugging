    ...
    <?php if ($client_info->address || (isset($client_info->custom_fields) && $client_info->custom_fields)) { ?>
        <div><?php echo nl2br($client_info->address); ?>
            ...
            <?php if ($client_info->state) { ?>
                <?php echo lang("state") . ": " . $client_info->state; echo",";echo lang("code") . ": " . $client_info->gstin_number_first_two_digits;  ?>
            <?php } ?>
            <?php
...
<?php } ?>
  ...
  text-align: left;padding: 5px;height: 30px;"><?php echo lang("buyers_order_no").":";?><?php if ($estimate_info->buyers_order_no) { ?>
            ...
            <?php } ?></td>
    <td style="border: 1px solid #dddddd;color: #666;font-size:14px;
  text-align: left;padding: 5px;height: 30px"><?php echo lang("buyers_order_date") . ": " ;?><?php if ($estimate_info->buyers_order_date) { ?><div style="font-weight: bold;color:#232323;"><?php echo format_to_date($estimate_info->buyers_order_date, false); ?><?php } ?></div></td>
  ...
  text-align: left;padding: 5px;height: 30px;"><?php echo lang("delivery_note_date").":";?> <?php if ($estimate_info->delivery_note_date) { ?><div style="font-weight: bold;color:#232323;"><?php echo format_to_date($estimate_info->delivery_note_date, false); ?></div><?php } ?></td>
  ...
  text-align: left;padding: 5px;">
<td style="border: 1px solid #dddddd;color: #666;font-size:14px;
  text-align: left;padding: 5px;height: 40px"><?php echo lang("dispatch_document_no").":" ;?><?php if ($estimate_info->dispatch_document_no) { ?><div style="font-weight: bold;color:#232323;"><?php echo $estimate_info->dispatch_document_no; ?></div>
            <?php } ?></td>
            <td   style="border: 1px solid #dddddd;color: #666;font-size:15px;
  text-align: left;padding: 5px;height:40px;"><?php echo lang("dispatched_through").":";?><?php if ($estimate_info->dispatched_through) { ?>
               <div style="font-weight: bold;color:#232323;"><?php echo $estimate_info->dispatched_through; ?></div>
            <?php } ?></td>
</tr>
  <tr style="border: 1px solid #dddddd;
  text-align: left;padding: 5px;">
