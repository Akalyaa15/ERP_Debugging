<?php echo form_open(get_uri("companys/save_file"), array("id" => "file-form", "class" => "general-form", "role" => "form")); ?>
<div class="modal-body clearfix">
    <input type="hidden" name="company_id" value="<?php echo $company_id; ?>" />
    <?php
    $this->load->view("includes/multi_file_uploader", array(
        "upload_url" =>get_uri("companys/upload_file"),
        "validation_url" =>get_uri("companys/validate_file"),
    ));
    ?>
</div>
    ...
    $(document).ready(function () {
        $("#file-form").appForm({
            onSuccess: function (result) {
                $("#company-file-table").appTable({reload: true});
            }
        });
