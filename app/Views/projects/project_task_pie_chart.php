...
foreach ($task_statuses as $status) {
   // $task_data[] = array("label" => $status->key_name ? lang($status->key_name) : $status->title, "data" => $status->total, "color" => $status->color);
    $task_data[] = array("label" => $status->key_name ? anchor(get_uri("projects/view/$status->project_id/tasks/$status->project_id/$status->status_id"),lang($status->key_name),array("class" => "black-link")) : anchor(get_uri("projects/view/$status->project_id/tasks/$status->project_id/$status->status_id"),$status->title,array("class" => "black-link")) , "data" => $status->total, "color" => $status->color);
}
?>
<script>
    $(function () {
