    ...
    <div class="panel">
        <?php if (isset($page_type) && $page_type === "full") { ?>
            <div class="page-title clearfix">
                <h1><?php echo lang('purchase_order_payments'); ?></h1>
            </div>
        <?php } else { ?>
            <div class="tab-title clearfix">
                <h4><?php echo lang('purchase_order_payments'); ?></h4>
            </div>
        <?php } ?>

        <div class="table-responsive">
            <table id="purchase_order-payment-table" class="display" width="100%">
            </table>
        </div>
    </div>
    <?php if (isset($page_type) && $page_type === "full") { ?>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $("#purchase_order-payment-table").appTable({
            source: '<?php echo_uri("purchase_order_payments/payment_list_data_of_vendor/" . $vendor_id) ?>',
            order: [[1, "desc"]],
            columns: [
                {title: '<?php echo lang("purchase_order_no") ?> ', "class": "w10p"},
