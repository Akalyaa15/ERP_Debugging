<?php echo form_open(get_uri("vendors/save_file"), array("id" => "file-form", "class" => "general-form", "role" => "form")); ?>
<div class="modal-body clearfix">
    <input type="hidden" name="vendor_id" value="<?php echo $vendor_id; ?>" />
    <?php
    $this->load->view("includes/multi_file_uploader", array(
        "upload_url" =>get_uri("vendors/upload_file"),
        "validation_url" =>get_uri("vendors/validate_file"),
    ));
    ?>
</div>
    ...
    $(document).ready(function () {
        $("#file-form").appForm({
            onSuccess: function (result) {
                $("#vendor-file-table").appTable({reload: true});
            }
        });
