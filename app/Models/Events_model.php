...
class Events_model extends Crud_model {
    ...
    function __construct() {
     ...
     function get_details($options = array()) {
        ...
        $start_time = get_array_value($options, "start_time");
        if ($start_time) {
            $where .= " AND $events_table.start_time='$start_time'";
        ...
        if ($user_id) {
           ...
           $is_partner = get_array_value($options, "is_partner");
            ...
            if ($is_partner) {
                ...
                OR $events_table.share_with='all_partners' 
                  OR (FIND_IN_SET('partner_contact:$user_id', $events_table.share_with)))";
            } else if ($is_client) {
                //client user's can't see the events which has shared with all team members
                $where .= " AND ($events_table.created_by=$user_id
                OR $events_table.share_with='all_clients'  
                OR (FIND_IN_SET('contact:$user_id', $events_table.share_with)))";
            }else if ($is_resource) {
                //client user's can't see the events which has shared with all team members
                $where .= " AND ($events_table.created_by=$user_id 
                OR $events_table.share_with='all_resource' 
                  OR (FIND_IN_SET('outsource_member:$user_id', $events_table.share_with))
                        $team_search_sql
                        )";
            }else {
    ...
    function get_detailss($options = array()) {
        ...
        if ($user_id) {
            ...
            if ($is_vendor) {
                //client user's can't see the events which has shared with all team members
                $where .= " AND ($events_table.created_by=$user_id
                 OR $events_table.share_with='all_vendors' 
                 OR(FIND_IN_SET('vendor_contact:$user_id', $events_table.share_with)))";
            }else {
                //searh for user and teams
                $where .= " AND ($events_table.created_by=$user_id 
                OR $events_table.share_with='all' 
        ...
        if ($vendor_id) {
            $where .= " AND $events_table.vendor_id=$vendor_id";
        }
        ...
        $sql = "SELECT $events_table.*,
            ...
            $vendors_table.company_name
        FROM $events_table
        LEFT JOIN $users_table ON $users_table.id = $events_table.created_by
        
        LEFT JOIN $vendors_table ON $vendors_table.id = $events_table.vendor_id       
        WHERE $events_table.deleted=0 $where
        ORDER BY $events_table.start_date ASC
        LIMIT $offset, $limit";
        return $this->db->query($sql);
