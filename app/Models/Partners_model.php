...
class Partners_model extends Crud_model {
    ...
    function get_details($options = array()) {
        ...
        if ($id) {
            $where = " AND $clients_table.client_id=$id";
        }
        ...
        FROM $clients_table
        LEFT JOIN $users_table ON $users_table.client_id = $clients_table.client_id AND $users_table.deleted=0 AND $users_table.is_primary_contact=1 
        LEFT JOIN (SELECT client_id, COUNT(id) AS total_projects FROM $projects_table WHERE deleted=0 GROUP BY client_id) AS project_table ON project_table.client_id= $clients_table.client_id
        LEFT JOIN (SELECT client_id, SUM(payments_table.payment_received) as payment_received, $invoice_value_calculation_query as invoice_value FROM $invoices_table
                ...
                LEFT JOIN (SELECT invoice_id, SUM(net_total) AS invoice_value FROM $invoice_items_table WHERE deleted=0 GROUP BY invoice_id) AS items_table ON items_table.invoice_id=$invoices_table.id AND $invoices_table.deleted=0 AND $invoices_table.status='not_paid'
                   ...
                   ) AS invoice_details ON invoice_details.client_id= $clients_table.client_id
        $join_custom_fieds               
 ...
 $delete_client_sqls = "UPDATE $clients_table SET $clients_table.deleted=1 WHERE $clients_table.id=$client_id; ";
        ...
        //delete contacts
        $delete_contacts_sql = "UPDATE $users_table SET $users_table.deleted=1 WHERE $users_table.client_id=$client_original; ";
