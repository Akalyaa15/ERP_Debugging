...
class Notes_model extends Crud_model {
    ...
    function get_details($options = array()) {
        ...
        if ($vendor_id) {
            $where .= " AND $notes_table.vendor_id=$vendor_id";
        }
        $company_id = get_array_value($options, "company_id");
        if ($company_id) {
            $where .= " AND $notes_table.company_id='$company_id'";
        }
        ...
        if ($my_notes) {
            $where .= " AND $notes_table.user_id=0 AND $notes_table.client_id=0 AND $notes_table.vendor_id=0 AND 
            $notes_table.company_id='0'"; //don't include client's and team member's notes
        }
