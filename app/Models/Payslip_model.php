    ...
    function get_details($options = array()) {
      ...
      $payslip_earnings_calculations = "(
       ...
       //status
        ...
        if ($status === "not_paid") {
            $where .= " AND $payslip_table.status !='draft' AND IFNULL(payments_table.payment_received,0)<=0";
        } else if ($status === "partially_paid") {
            $where .= " AND IFNULL(payments_table.payment_received,0)>0 AND IFNULL(payments_table.payment_received,0)<$payslip_table.total";
        } else if ($status === "fully_paid") {
            $where .= " AND TRUNCATE(IFNULL(payments_table.payment_received,0),2)>=$payslip_table.total";
        }
...
function get_emp_monthly_payslip_info_suggestion($month = "",$user_id = "") {
    ...
    function get_payslip_user_id_suggestion($keyword = "",$month="",$year="",$saved_user_id="") {
        $users_table = $this->db->dbprefix('users');
        
   $payslip_table = $this->db->dbprefix('payslip');
   
   // check this this payslip user id from payslip table
   if($saved_user_id){ 
 $sqls = "SELECT $payslip_table.user_id
        FROM $payslip_table
 WHERE $payslip_table.deleted=0 AND MONTH($payslip_table.payslip_date) ='$month'  AND YEAR($payslip_table.payslip_date) ='$year' AND  $payslip_table.user_id !=$saved_user_id
        ";
    }else{
  $sqls = "SELECT $payslip_table.user_id
        FROM $payslip_table
...
function get_payslip_user_per_month_total_duration($payslip_id = 0) {
    ...
    //get payslip table details
        ...
        $payslip_team_member_job_table_sql = "SELECT $team_member_job_info_table.*
        FROM $team_member_job_info_table
        LEFT JOIN $payslip_table ON $payslip_table.user_id=$team_member_job_info_table.user_id
        WHERE $payslip_table.deleted=0 AND $team_member_job_info_table.deleted=0 AND $payslip_table.id=$payslip_id";
        $payslip_team_member_job_table_list = $this->db->query($payslip_team_member_job_table_sql)->row();

        $date=$payslip_table_list->payslip_date;
       ...
       // get users attendance details per month 
        ...
        // get user no of joining date 
        $teammembers_joining_date = "";
        $teammembers_joing_no_days = 0;
        if($payslip_team_member_job_table_list->date_of_hire&&$payslip_team_member_job_table_list->date_of_hire!='0000-00-00'){
            //$teammembers_joing_to_now_date = $payslip_team_member_job_table_list->date_of_hire."_".$last_day;
            $teammembers_joining_date = $payslip_team_member_job_table_list->date_of_hire;
            $teammembers_joing_start = new DateTime($payslip_team_member_job_table_list->date_of_hire);
            $teammembers_joing_end = new DateTime($last_day);
            if($teammembers_joing_start<$teammembers_joing_end){
            $teammembers_joing_diff = $teammembers_joing_start->diff($teammembers_joing_end);
            $teammembers_joing_no_days = $teammembers_joing_diff->days + 1;
            }else{
                $teammembers_joing_no_days = 0;
            }
        ...
        // payable days 
   $users_join_date_new =date('Y-m', strtotime($teammembers_joining_date));
   $payslip_date_pervious_first =date('Y-m', strtotime($first_day));
   $users_month_payable_days = Date('t', strtotime($firstdayMonth));
    if($users_join_date_new ==$payslip_date_pervious_first){

            if($teammembers_joing_start<$teammembers_joing_end){
            $teammembers_joing_diff = $teammembers_joing_start->diff($teammembers_joing_end);
            $users_month_payable_days = $teammembers_joing_diff->days + 1;
        }else{
            $users_month_payable_days =0;
        }

    }
    //end payable days
        
// end get user no of joining date
        ...
        $payslip_info_holiday_of_week = $payslip_table_list->holiday_of_week;
        ...
        if($payslip_table_list->payslip_casual_leave == '0'){
            ...
            $payslip_info_casual_leave_first =number_format($teammembers_joing_no_days/get_setting('maximum_no_of_casual_leave_per_month'), 2, '.', '');
        }else{
            //$payslip_info_casual_leave_first = $payslip_table_list->payslip_casual_leave;
            $payslip_info_casual_leave_first = number_format($teammembers_joing_no_days/$payslip_table_list->payslip_casual_leave, 2, '.', '');
        }
       ...
       //annaual leave  clculation 
        ...
        $annaual_leave_start_date = $teammembers_joining_date;
        $annaual_leave_end_date =  $annual_last_day;
        $annaual_leave_where .= " AND ($payslip_table.payslip_date >='$annaual_leave_start_date' AND $payslip_table.payslip_date <= '$annaual_leave_end_date')";
        ...
        WHERE $payslip_table.id!=$payslip_id AND $payslip_table.deleted =0 AND $payslip_table.user_id =$payslip_table_list->user_id $annaual_leave_where";
        $annual_leave_date = $this->db->query($annual_leave_date_sqls)->row();
        $annual_leave_taken_user =number_format($annual_leave_date->total_annual_paid_leave, 2, '.', '');
       // END ANNUAL LEAVE 
  ...
  if($payslip_info_casual_leave_first != 0){
    ...
    if($payslip_table_list->payslip_created_status != "create_timesheets"){ //start attendance based payslip
       ...
       // get users attendance details per month 
        ...
        // this month new joining user
        if($users_join_date_new ==$payslip_date_pervious_first){
        $first_day= $teammembers_joining_date;
       }
...
$todt=date('Y-m-d ',strtotime("Last Day of $monthName $years"));
 ...
 // this month new joing user check
if($users_join_date_new ==$payslip_date_pervious_first){
        $fromdt=$teammembers_joining_date;
   }
/*$num_sundays='';                
...
$country_holidays_array = explode(",", "");
//$branch_holidays_array = explode(",", $user_branch_result->holiday_of_week);
$branch_holidays_array = explode(",",$payslip_info_holiday_of_week);
...
$leave_last_month=Date('m', strtotime($currentMonth ));
      ...
      if ($leave_start_date && $leave_end_date) {
           ...
           // this month new joing user  check
           if($users_join_date_new ==$payslip_date_pervious_first){
          $leave_where .= " AND ($leave_applications_table.start_date <='$leave_year-$leave_last_month-31' AND $leave_applications_table.end_date >= '$teammembers_joining_date') ";
         } 
        }
...
$leave_last_month=Date('m', strtotime($currentMonth ));
      ...
      if ($leave_start_date && $leave_end_date) {
        ...
        $official_leave_where .= " AND ($events_table.start_date  BETWEEN '$teammembers_joining_date' AND '$leave_year-$leave_last_month-31')  ";
    } 
        } 
...
//get no of official holidays from events table 
  ...
  // get payslip payments
        $payment_sql = "SELECT SUM($payslip_payments_table.amount) AS total_paid
        FROM $payslip_payments_table
        WHERE $payslip_payments_table.deleted=0 AND $payslip_payments_table.payslip_id=$payslip_id";
        $payslip_payment = $this->db->query($payment_sql)->row();  
...
*/
        ...
        $result = new stdClass();
$result->payslip_total_paid = $payslip_payment->total_paid;
$result->monthly_eligible_leave_from_joining =
$payslip_info_casual_leave_first;
        //annual  paid leave 
$result->total_annual_paid_leave =  $annual_leave_taken_user;
$result->total_payable_days_inmonth = $users_month_payable_days;
        //annual  paid leave 
...
if($payslip_info_holiday_of_week||$payslip_info_holiday_of_week==0){
$branch_weekoff_array = explode(",", $payslip_info_holiday_of_week);
    //$branch_weekoff_array = array($user_branch_result->holiday_of_week);

    $branch_weekoff_count  = count($branch_weekoff_array);
}else{
    $branch_weekoff_count = 0;
}

if(7>$branch_weekoff_count){
  $branch_weekworkdays = (7-$branch_weekoff_count);
  $branch_weekoff = (7-$branch_weekworkdays);  
}else if(7==$branch_weekoff_count){
    $branch_weekworkdays = 7;
    $branch_weekoff = $branch_weekworkdays; 
}
...
}else if($result->total_user_days<18&&$result->total_user_days>=$branch_weekworkdays){

$userdays_dividesixdays = $result->total_user_days/$branch_weekworkdays; //this result divide by six days weekly one sunday enable 
if($num_sundays){
  $employee_contribute_give_official_leave = intval($userdays_dividesixdays*$branch_weekoff)+$result->official_leave;  
}else{
    $employee_contribute_give_official_leave = $result->official_leave;
}


}else if($result->total_user_days<$branch_weekworkdays && $result->total_user_days>0){
...
// this month new joining user check lop days 
if($users_join_date_new ==$payslip_date_pervious_first){
$result->minus_new_joing_date = (($result->num_of_days-$users_month_payable_days)*$company_working_hours_for_one_day);
$new_user_convert_days = (($monthly_lop_hours-$result->minus_new_joing_date)/$company_working_hours_for_one_day);
$result->monthly_lop_days = number_format($new_user_convert_days, 3, '.', '');
}
...
return $result;
       ...
       // get users attendance details per month 
        ...
        // this month new user joining check 
        if($users_join_date_new ==$payslip_date_pervious_first){
        $first_day= $teammembers_joining_date;
       }
...
// new user joining check this month
if($users_join_date_new ==$payslip_date_pervious_first){
        $fromdt= $teammembers_joining_date;
   }
...
$leave_last_month=Date('m', strtotime($currentMonth ));
      ...
      if ($leave_start_date && $leave_end_date) {
           ...
           $official_leave_where .= " AND ($events_table.country_permission='all' 
               OR (FIND_IN_SET('$user_branch_result->buid', $events_table.branch)))";
           $official_leave_where .= " AND ($events_table.start_date  BETWEEN '$leave_year-$leave_last_month-01' AND '$leave_year-$leave_last_month-31')  "; 
           // this month new user joining date check
           if($users_join_date_new ==$payslip_date_pervious_first){
...
//monthly lop hour  to lop days 
$result->monthly_lop_days = number_format($monthly_lop_hours/$company_working_hours_for_one_day, 3, '.', ''); 
// this month  new user joining check lop days  
if($users_join_date_new ==$payslip_date_pervious_first){
$result->minus_new_joing_date = (($result->num_of_days-$users_month_payable_days)*$company_working_hours_for_one_day);
$new_user_convert_days = (($monthly_lop_hours-$result->minus_new_joing_date)/$company_working_hours_for_one_day);
$result->monthly_lop_days = number_format($new_user_convert_days, 3, '.', '');
...
if($payslip_ot_permission =="specific" && (in_array('member:'.$payslip_user_id,$ot_permission_user_array)||in_array($payslip_user_id,$ot_team_group_list_array))){
  ...
  $ot_result = "no";
       ...
       //payslip user get salary and deductions amount 
public function get_payslip_user_deductions_from_branch_settings($payslip_id){
        $payslip_table = $this->db->dbprefix('payslip');
        $payslip_table_sql = "SELECT $payslip_table.*
        FROM $payslip_table
        WHERE $payslip_table.deleted=0 AND $payslip_table.id=$payslip_id";
        ...
        $branch_deductions_table = $this->db->dbprefix('country_deductions');
        $branch_earnings_table = $this->db->dbprefix('country_earnings');
        $settings_deductions_table = $this->db->dbprefix('deductions');
        $settings_earnings_table = $this->db->dbprefix('earnings');
        $users_table = $this->db->dbprefix('users');
        $team_member_job_info_table = $this->db->dbprefix('team_member_job_info');
        $branch_table = $this->db->dbprefix('branches');
        $country_table = $this->db->dbprefix('country');

        // get buid in user table 
        $user_data_sql = "SELECT $users_table.*
        FROM $users_table
        LEFT JOIN $payslip_table ON $payslip_table.user_id= $users_table.id  
       WHERE $users_table.deleted=0 AND $payslip_table.id=$payslip_id AND $payslip_table.deleted=0";
        $user_table_list = $this->db->query($user_data_sql)->row();

        // get branch table 
        $user_buid_data_sql = "SELECT $branch_table.*
        FROM $branch_table
        WHERE $branch_table.deleted=0 AND $branch_table.buid=
        '$user_table_list->buid'";
        $user_branch_table_list = $this->db->query($user_buid_data_sql)->row(); 

       


        if($user_branch_table_list->id){
        // get branch earnings table 
        $branch_earnings_sql = "SELECT $branch_earnings_table.*
        FROM $branch_earnings_table
        WHERE $branch_earnings_table.deleted=0 AND $branch_earnings_table.status='active' AND $branch_earnings_table.key_name='basic_salary' AND 
        $branch_earnings_table.country_id=$user_branch_table_list->id";
        $earnings_result = $this->db->query($branch_earnings_sql)->row(); 

        $branch_deductions_sql = "SELECT $branch_deductions_table.*
        FROM $branch_deductions_table
        WHERE $branch_deductions_table.deleted=0 AND $branch_deductions_table.status='active' AND 
        $branch_deductions_table.country_id=$user_branch_table_list->id";
        $deductions_result = $this->db->query($branch_deductions_sql)->result();
       
       }else{
            $settings_earnings_sql = "SELECT $settings_earnings_table.*
        FROM $settings_earnings_table
        WHERE $settings_earnings_table.deleted=0 AND $settings_earnings_table.status='active' AND $settings_earnings_table.key_name='basic_salary'";
        $earnings_result = $this->db->query($settings_earnings_sql)->row(); 

        $settings_deductions_sql = "SELECT $settings_deductions_table.*
        FROM $settings_deductions_table
        WHERE $settings_deductions_table.deleted=0 AND $settings_deductions_table.status='active'";
        $deductions_result = $this->db->query($settings_deductions_sql)->result();
        }

        $result = new stdClass();
        $salary=$payslip_info_salary/100;
        $b=$salary *$earnings_result->percentage;
        $c = $b/100; 
        $total=0;
        foreach ($deductions_result as $deduction) {
         $a=$c * $deduction->percentage ;
         $total+=$a;
            
        }
        $result->branch_deductions_amount =$total;
...
//change the invoice status from draft to not_paid
    function set_payslip_status_to_not_paid($payslip_id = 0) {
        $status_data = array("status" => "not_paid");
        return $this->save($status_data, $payslip_id);
    }
