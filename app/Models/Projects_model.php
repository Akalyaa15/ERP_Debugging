...
$project_manager= get_array_value($options, "project_manager");
        if ($project_manager) {
            $extra_join = " LEFT JOIN $project_members_table ON $project_members_table.project_id= $projects_table.id";
            $where .= " AND $project_members_table.user_id=$project_manager AND $project_members_table.is_project_manager=1 AND $project_members_table.is_leader=0 AND $project_members_table.is_purchase_manager=0 AND $project_members_table.deleted=0";
        }
        ...
        $sql = "SELECT $projects_table.*,$country_table.countryName, $clients_table.company_name, $clients_table.currency_symbol,  total_points_table.total_points, completed_points_table.completed_points $select_custom_fieds
        FROM $projects_table
        LEFT JOIN $clients_table ON $clients_table.id= $projects_table.client_id
        LEFT JOIN $country_table ON $country_table.id= $clients_table.country
