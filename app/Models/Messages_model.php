...
class Messages_model extends Crud_model {
    ...
    function get_g_notifications($user_id, $last_message_checke_at = "0", $active_message_id = 0) {
        $messages_table = $this->db->dbprefix('group_comments');
        $users_table = $this->db->dbprefix('users');

        $where = "";
        // if ($active_message_id) {
        //     $where = " AND $messages_table.comment_id!=$active_message_id";
        // }



        $sql = "SELECT $messages_table.id, $messages_table.comment_id, $messages_table.created_at, CONCAT($users_table.first_name, ' ', $users_table.last_name) AS user_name, $users_table.image AS user_image
        FROM $messages_table
        LEFT JOIN $users_table ON $users_table.id=$user_id
        WHERE $messages_table.deleted=0  AND FIND_IN_SET($user_id, $messages_table.group_members) != 0  
        AND timestamp($messages_table.created_at)>timestamp('$last_message_checke_at')  
        AND  FIND_IN_SET($user_id, $messages_table.read_members) = 0 $where
        ORDER BY timestamp($messages_table.created_at) DESC";
        return $this->db->query($sql);
    }
