...
class Tickets_model extends Crud_model {
    ...
    function get_details($options = array()) {
        ...
        if ($status === "closed") {
            ...
            $where .= " AND FIND_IN_SET($tickets_table.status, 'open,client_replied')";
        }if ($status === "new") {
            $where .= " AND FIND_IN_SET($tickets_table.status, 'new')";
        }
        ...
        $sql = "SELECT $tickets_table.*, $ticket_types_table.title AS ticket_type, $clients_table.company_name, $project_table.title AS project_title,
              CONCAT($users_table.first_name, ' ',$users_table.last_name) AS assigned_to_user, $users_table.image as assigned_to_avatar,$users_table.user_type as assigned_to_user_type $select_custom_fieds
        FROM $tickets_table
