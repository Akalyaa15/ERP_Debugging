...
class Service_id_generation_model extends Crud_model {

    private $table = null;

    function __construct() {
        $this->table = 'service_id_generation';
        parent::__construct($this->table);
    }

     function get_details($options = array()) {
      $product_id_generation_table = $this->db->dbprefix('service_id_generation');
        $part_no_generation_table = $this->db->dbprefix('job_id_generation');
        $where = "";
        $id = get_array_value($options, "id");
        if ($id) {
            $where = " AND $product_id_generation_table.id=$id";
        }
        $product_id = get_array_value($options, "service_id");
        if ($product_id) {
...
function get_service_id_suggestion($keyword = "") {
        $product_id_generation_table = $this->db->dbprefix('service_id_generation');
...
$inventory_item=str_ireplace("]",")",$vv);
    ...
    function get_service_id_info_suggestion($item_name = "") {
     ...
     function is_service_id_generation_exists($title, $id = 0) {
